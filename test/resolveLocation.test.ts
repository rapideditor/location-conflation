...
describe('resolveLocation', () => {
  ...
  describe('points', () => {
    it('valid [lon, lat] Array returns a feature match', () => {
    ...
    it('valid [lon, lat, radius] Array returns a feature match', () => {
    ...
    it('invalid [lon, lat] Array throws an error', () => {
  ...
  describe('`.geojson` filenames', () => {
    it('known `.geojson` filename with id returns a feature match', () => {
    ...
    it('known `.geojson` filename with id property returns a feature match', () => {
    ...
    it('invalid `.geojson` filename throws an error', () => {
  ...
  describe('country coder feature identifiers', () => {
    it('valid country coder feature identifier returns a feature match', () => {
      const location = 'gb';
      const result = loco.resolveLocation(location);
    ...
    it(`valid country coder world feature identifier returns our world feature, not country-coder's`, () => {
      const location = '001';
      const result = loco.resolveLocation(location);
      expect(result.type).toBe('countrycoder');
      expect(result.location).toBe(location);
      expect(result.feature.id).toBe('Q2');
      expect(result.feature.properties.id).toBe('Q2');
      expect(result.feature.geometry.coordinates).toEqual(
        [[[-180, -90], [180, -90], [180, 90], [-180, 90], [-180, -90]]]
      );
    });

    it('invalid country coder feature identifier throws an error', () => {
      const location = 'fake';
      expect(() => loco.resolveLocation(location)).toThrow(/invalid location/i);
    });
  });
