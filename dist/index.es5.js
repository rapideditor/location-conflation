...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.LocationConflation = factory());
...
function loadDerivedDataAndCaches(borders) {
  ...
  for (var i$1 in borders.features) {
    ...
    feature$1.properties.groups = feature$1.properties.groups.map(function (groupID) {
      return featuresByCode[groupID].properties.id;
    });
    loadMembersForGroupsOf(feature$1);
  }
  for (var i$2 in borders.features) {
    var feature$2 = borders.features[i$2];
    loadRoadSpeedUnit(feature$2);
    loadRoadHeightUnit(feature$2);
    loadDriveSide(feature$2);
    loadCallingCodes(feature$2);
    loadGroupGroups(feature$2);
  }
  for (var i$3 in borders.features) {
    var feature$3 = borders.features[i$3];
    feature$3.properties.groups.sort(function (groupID1, groupID2) {
      return (
        levels.indexOf(featuresByCode[groupID1].properties.level) -
        levels.indexOf(featuresByCode[groupID2].properties.level)
      );
    });
    if (feature$3.properties.members)
      { feature$3.properties.members.sort(function (id1, id2) {
        var diff =
  ...
  function loadGroupGroups(feature) {
    ...
    for (var i in props.members) loop( i );
  ...
  function loadRoadHeightUnit(feature) {
    var props = feature.properties;
    if (feature.geometry) {
  ...
  function loadCallingCodes(feature) {
    var props = feature.properties;
  ...
  function cacheFeatureByIDs(feature) {
    ...
    for (var i in ids) {
      var id$1 = canonicalID(ids[i]);
      featuresByCode[id$1] = feature;
    }
  }
}
...
function featureForLoc(loc, opts) {
  ...
  for (var i in features) {
    ...
    var levelIndex = levels.indexOf(feature.properties.level);
    if (
...
function aggregateFeature(id) {
  var features = featuresIn(id, false);
...
defaultExport.prototype.resolveLocation = function resolveLocation (location) {
  ...
  // A .geojson filename?
    ...
    var feature$1$1 = _cloneDeep(feature(id));
...
//
defaultExport.prototype.validateLocationSet = function validateLocationSet (locationSet) {
...
defaultExport.prototype.resolveLocationSet = function resolveLocationSet (locationSet) {
  ...
  // Calculate unions
  var includeGeoJSON = _clip(includes.map(function (d) { return d.feature; }), 'UNION');
  var excludeGeoJSON = _clip(excludes.map(function (d) { return d.feature; }), 'UNION');
...
// strict
//
defaultExport.prototype.strict = function strict (val) {
...
function _clip(features, which) {
  ...
  var fn = { UNION: index.union, DIFFERENCE: index.difference }[which];
  var args = features.map(function (feature) { return feature.geometry.coordinates; });
